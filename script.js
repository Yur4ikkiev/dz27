...
class User{
	...
	renderCourses() {
		return this.courses.length > 0 ? this.courses.map(({title, mark, lector}) => `
		<div class="user__courses">
			<p class="user__courses--course ${this.role}">${title} 
				<span class="${this.renderMark(mark)}">${this.renderMark(mark)}</span>
			</p>
		</div>
		`).join(''): '';
	}
	...
	renderCourses() {
		...
		<div class="user__courses admin--info">
			<div class="user__courses--course lector">
				...
				<p>Average student's score: <span class="${this.renderMark(studentsScore)}">${this.renderMark(studentsScore)}</span></p>
			</div>
		</div>
		`).join('') : '';
	}
